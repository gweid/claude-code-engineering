...
主对话只看到子代理所返回的结论 ，不需要承载 500 行的搜索过程。这意味着对话不会因为一次大搜索就把 token 配额用光，重要的讨论内容不会被中间输出淹没，Claude 在后续对话中能更好地“记住”关键信息。
...
**约束，解决的是行为不可控问题**。通过工具权限边界，把“希望别这么做”变成“物理上做不到”，让代码审查只能读、修 bug 才能写，角色职责不再依赖提示词自觉。
...
比如让 Claude 审查代码，只希望它看，不希望它动手改。如果没有权限边界，Claude 可能在审查过程中顺手“修”了一些它认为有问题的代码。这不是想要的行为。
...
> PS: claude code 自 v2.1.172 起支持嵌套，最大 5 层
...
其实目前大部分 agent 系统都不支持子代理嵌套调用子代理，这是架构设计问题，主要原因：

- **复杂度指数爆炸**。每增加一层嵌套，系统的**状态空间、分支路径、交互组合**都呈指数增长。一个 3 层嵌套、每层 5 个 agent 的系统，理论上有 125 条执行路径，而实际由于 LLM 的非确定性，真实路径可能远超此数
- **上下文窗口的"信息衰减"**。每一层都需要**将上层意图"翻译"成子任务描述**，这个过程必然有信息损失，到了第 3-4 层，原始用户意图可能已经被严重扭曲，会稀释模型注意力
- **时间与成本陡增**。每层嵌套都需要完整的 system prompt + 上下文传递，token 消耗呈乘法增长。嵌套调用会**串行阻塞**
- **错误传播与放大**。错误不可逆，重试代价过高
- **可观测性与调试的噩梦**。需要追踪**树状调用链**，日志量呈指数增长
- **循环依赖风险**。LLM 不具备可靠的"全局调用图感知"能力，无法自主避免循环

所以，不支持嵌套不是"做不到"，而是**刻意的设计取舍**。非必要情况下，不建议嵌套使用子代理
...
---
name: code-reviewer
description: 审查代码中的安全问题和最佳实践。在代码变更后使用。
tools: Read, Grep, Glob
model: sonnet
---
...
skills 字段允许在子代理启动时，把指定 Skill 的完整内容注入到子代理的上下文中。这意味着子代理不需要在执行过程中发现和加载 Skill，知识已经在它的脑子里了。

```yaml
---
name: impact-analyzer
description: 分析代码更改对完整调用链的影响范围。
tools: Read, Grep, Glob, Bash
skills:
...
子代理不会 自动继承主对话中可用的 Skill。如果希望子代理拥有某个 Skill 的知识，必须在这里显式列出
...
一个基础的、好的子代理设计，基本包括五大部分：

- 元数据与工具约束 (Metadata & Tools)
- 领域边界定义 (Your Domain)
- 执行策略 (When Invoked)
...
这不是保守，而是工程智慧。每增加一个 Agent，就增加了一层调试复杂度、一份 token 成本、和一个潜在的失败点。但当任务真的跨越了单 Agent 的能力边界时，正确的多 Agent 架构会带来巨大性能提升
...
LangChain 官方归纳了四种核心的 Multi-Agent 设计模式：
...
Sub-Agent 和 Skills 这两种模式都是 Claude Code 原生的，很容易理解，但是 Hand-off 如何实现？
...
基于上面，可以总结从单一Agent到复杂智能体系统设计的黄金法则：

1. 从单 Agent 开始 → 只在遇到明确瓶颈时才升级
2. 先加工具，再加 Agent → Tools 是最小的扩展单位
3. 选对模型 > 堆更多 token → 升级模型的效果超过翻倍预算
4. 上下文隔离是核心价值 → 多 Agent 的第一价值不是并行，是隔离
5. Token 成本要高价值任务 → 不是所有场景都值得 Multi Agent
从 Sub-Agent 到 Multi-Agent，不是一个线性的“升级”过程，而是一个根据任务特征选择合适架构的工程决策。

**最好的架构不是最复杂的架构，而是恰好满足需求的最简架构**。当能用一个 Agent + 几个 Tool 解决问题时，就不需要引入 Supervisor + SubAgents 的复杂度。但当任务的并行性、专业性和上下文管理需求确实超越了单 Agent 的能力边界时，正确的多 Agent 架构会带来显著的质量提升
...
一个经验法则：假如一个命令的输出超过 50 行（行数也还要视具体情况而定），只需要关心其中不到 10 行（也就是不到五分之一）的内容，就应该用子代理
...
三个子代理配置在目录 `02-SubAgents/06-parallel-explore` 下
...
可以看到四个子代理依次执行，每个阶段返回简洁的报告，主对话保持清洁。对于复杂 bug、需要系统性排查的情况，流水线尤其有价值。而且每个子代理职责清晰，便于追踪问题；且权限递进，只有必要时才给写权限
...
流水线的架构约束：**子代理不能嵌套，流水线的编排者只能是主对话**

> claude code v2.1.172 起，支持子代理调用子代理

在设计流水线之前，有一个关键约束必须了解：子代理不能生成子代理 bug-locator 不能自己去调用 bug-analyzer。
...
- **调试更容易**：出了问题，你知道每个阶段的输入输出分别是什么，不会出现“子代理 A 调了子代理 B，B 又调了 C，结果在 C 里出了错但只看到 A 的输出”这种黑盒嵌套
...
Claude Code 提供了 **Resume 机制**，每个子代理执行完后都有一个 Agent ID，可以用这个 ID 恢复它的完整上下文。

对于流水线来说，这意味着，如果四阶段流水线跑到第三阶段时服务器重启了，只需要：
...
流水线的核心不是简单分成多个阶段，而是**阶段间的交接契约**。设计信息必须完整传递，不能让下游重复上游的工作。

主对话不是旁观者，而是**编排者**。它负责审查、决策、介入和回退。流水线失败时，回退到分析阶段而非重试执行阶段。这是避免死循环的关键。

真实任务通常是**混合模式**。有三种混合模式的判断标准：
...
之前的子代理工作模式虽然也各有差异，但是基本模式一致。都是主对话像老板一样，把任务委派 给各个专职员工（子代理）。这种模式解决了上下文污染、权限边界、任务并行等问题。但它有一个**根本性的限制：子代理只能向主对话汇报，不能互相交流**。

![](./imgs/img44.png)

这在很多场景下足够好用。但在实际生产环境中可能会遇到更复杂、更诡异的场景：比如多假设并行验证。

比如：系统出现了一个奇怪 bug，用户登录后偶尔会话丢失，没有明确的规律。怀疑可能是：
...
Agent Teams 可以协调多个 Claude Code 实例作为一个团队工作。一个会话作为 Team Lead（团队领导），协调工作、分配任务、综合结果。Teammates（队友） 各自独立工作，每个都有自己的上下文窗口，并且可以直接互相通信。
...
**Claude 不会未经批准就创建 team**




使用 prompt 告诉 Claude 创建团队并描述任务：
...
Claude 会建团队，生成指定的 Teammates 探索问题，然后综合各方发现，完成后清理团队
...
团队配置包含 members 数组，记录每个 Teammate 的名称、agent ID 和类型。Teammates 可以读取这个文件来发现其他团队成员。这些都是由 Claude Code 自行搞定
...
最后，全部工作收尾，Lead 会清理团队，移除共享的团队资源。需要注意，如果出现需要用户人工指示清理的化，清理前先关闭所有 Teammates，而且应该只让 Lead 执行清理，因为Teammates 的 team 上下文可能不正确
...
这里使用 agent team 的原因：单独调查者容易“锚定”，找到一个 bug 后就满意了，不继续挖掘。而bug 之间的关联需要跨视角发现，只有Session 侦探和数据库侦探各自的发现互相对照，才能看到级联效应。此外，架构侦探可以把其他侦探的发现串联起来，通过辩论机制产出更完整的根因链
...
```yaml
agent-teams-bug-hunt
├── .claude/
│    └── settings.json    # claude code 开启 CLAUDE_CODE_EXPERIMENTAL_AGENT_TEAMS
├── buggy-app             # 六个代码文件，一系列关联 bug
├── hooks                 # Teammateldle 和 TaskCompleted Hook 示例脚本
├── bug-report.md         # 模拟用户报告的三个症状（调查起点）
├── team-prompt.md        # 三种启动指令（竞争假设/Delegate Mode/Plan Approval)
└── findings-template.md  # 标准化的调查报告模板
```

将 `team-prompt.md` 中的某一种启动模式（里面有三种模式）拷贝到 Claude Code 命令行中，启动 Teams 即可。

当前场景下推荐使用：**竞争假设模式**。prompt 如下：
...
最终综合所有发现，生成一份按照 findings-template.md 格式的调查报告，输出到当前目录下的 investigation-report.md 文件。
```
...
Agent Teams 最强大的地方在于它支持的协作模式，主要是四种核心设计模式：
...
这种模式适用于**代码审查**、PR Review 等需要从多个维度同时评估的任务
...
这种模式适用于**新功能开发**，涉及多个独立模块（前端、后端、数据库、测试）的场景
...
Sub-Agents 和 Agent Teams这两种架构选择决策：
...
Claude指出，Agent Teams 使用的 token 显著高于单会话或子代理，因为每个 Teammate 是独立的 Claude 实例，有独立的上下文窗口。消息通信消耗额外 token，而且团队越大，成本越高
...
Sub-Agents 解决的是“工程控制问题”，也就是“单脑如何安全分工”。常见的子代理的四种使用模式：
...
Sub-Agents 应用场景是：**上下文污染 、权限边界 、高噪声干扰 、阶段性分工和可控编排**
...
Agent Teams 应用场景是：**锚定偏见 、单视角误判 、过早收敛和决策质量不足等**
...
sub-agent +  agent-team 的综合场景运用




**场景**：电商系统在大促期间出现了“支付超时”问题，需要排查并修复
