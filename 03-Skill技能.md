...
你是 Java 现代化改造专家。改造代码时严格遵循预加载的 java-modernization 规范。每完成一个文件的改造后，立即使用 java-refactor-check 的流程自检。
```

SubAgent 是“谁来做”，Skill 是“怎么做”。组合后就成了一个懂团队规范的改造专家
...
第一层：知识工程，让 AI 在正确的时刻拥有正确的知识：

```yaml
skill 结构与触发机制：知识的结构与触发
    ↓ 知识怎么写、什么时候被加载
任务型 skill 实战：知识的操作化
    ↓ 知识怎么变成可执行的标准流程
渐进式披露架构：知识的经济学
...
这一层解决的是**知识管理问题** 。团队有大量专业知识（编码规范、架构约定、审查标准、部署流程），怎么让 AI 在需要时精准获取，不需要时不浪费上下文？



第二层：系统集成，让知识驱动整个智能体系统：

```yaml
skill 与 subagent 配合：知识 × 智能体
    ↓ Skill 怎么和 SubAgent 组合成领域专家
skill 架构定位与设计模式：知识在架构中的位置
